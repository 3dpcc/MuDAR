...
class AncestralModel(l.LightningModule):
    def __init__(self, cfg):
        ...
        self.encoder0 = nn.Embedding(self.cfg["ntoken"], 48)
        self.encoder1 = nn.Embedding(self.cfg["nlevel"],  4)
        self.encoder2 = nn.Embedding(self.cfg["noctant"], 4)
        self.encoder3 = nn.Embedding(self.cfg["nlaser"],  4)
        self.encoder4 = nn.Embedding(self.cfg["nphi"],    4)
    ...
    def forward(self, source):
        ...
        laser_emb  = self.encoder3(laser)
        phi_emb    = self.encoder4(phi)

        # local geometric feature extraction
        parent_pos_emb = self.pos_emb(occupy, level, octant, laser, phi, pos)

        emb = torch.cat([occupy_emb, level_emb, octant_emb, laser_emb, phi_emb], dim=-1).reshape(l, b, -1)
