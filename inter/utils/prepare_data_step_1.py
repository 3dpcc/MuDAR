...
if __name__ == "__main__":
    ...
    for idx in range(0, 5):
        for seq in range(0, 12):
            point_paths = glob.glob(f"/zhu/data/NuScenes/train/pccs_raw/{idx:02d}/{seq:02d}**/*", recursive=True)
            ...
            to_save_path = f"/zhu/data/NuScenes/train/process_rpcc_pccs/{idx:02d}/{seq:02d}"
            os.makedirs(to_save_path, exist_ok=True)

            # for path in point_paths:
            #     process_point_clouds(path, to_save_path, 16)

            joblib.Parallel(
                n_jobs=64, verbose=10, pre_dispatch="all"
            )(
                [
                    joblib.delayed(process_point_clouds)(point_path, to_save_path, 16)
